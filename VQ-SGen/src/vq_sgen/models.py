"""Model definitions and core training routines."""
...
def train_generator(model, train_loader, val_loader, cfg, device):
    from .pipeline import run_generator_epoch
    ...
    for epoch in range(1, int(cfg["generator"]["epochs"]) + 1):
        ...
        train_metrics = run_generator_epoch(
            model, train_loader, device, cfg, optimizer=optimizer,
        ...
        val_metrics = run_generator_epoch(
            model, val_loader, device, cfg, optimizer=None,
            mixed_precision=bool(cfg["project"]["mixed_precision"]),
            grad_clip_norm=float(cfg["generator"]["grad_clip_norm"]),
...
class StrokeAutoregressiveGenerator(nn.Module):
    def __init__(
        ...
        # For QuickDraw, class-token-only conditioning can weaken later in decoding.
        # Use class token + global class bias + output FiLM together for stable conditioning.
