...
from .artifacts import save_trained_artifacts
from .pipeline import run_v18_pipeline
...
# Keep debug visualizations available after both train/test runs.
...
# -------------------------------
# 2) Pretrained usage for the 5 model stages
#    - True  + train=False : load pretrained only
#    - True  + train=True  : initialize from pretrained and finetune
#    - False + train=True  : train from scratch
#    - False + train=False : invalid
# -------------------------------
...
# -------------------------------
# 3) Dataset preset assignment per model stage
#    - This project uses QuickDraw only.
#    - All stages default to "target_quickdraw".
# -------------------------------
...
# CreativeSketch download / cache options
...
# -------------------------------
# 5) Source / target dataset preset options
# -------------------------------
...
# QuickDraw canonical stroke order
# - "none": keep original order
# - "top_to_bottom": sort by centroid (top to bottom, then left to right)
# - "bbox_area_desc": sort by bbox area descending, then top to bottom, then left to right
QUICKDRAW_CANONICAL_STROKE_ORDER = "bbox_area_desc"

# ===== Detailed Hyperparameters / Auto cfg build =====
...
def _load_external_config_overrides():
    config_path = os.environ.get("VQ_SGEN_CONFIG", "").strip()
    if not config_path:
        default_path = os.path.join(os.getcwd(), "configs", "config.json")
    ...
    print(f"Loaded config overrides: {config_path}")
    return overridden


def _apply_derived_defaults(overridden_keys):
    global NOTEBOOK_NAME
    global DRIVE_MODEL_ROOT
    global DRIVE_MODEL_VER_DIR
    global WORKSPACE_ROOT
    global RUN_TAG
    global PREPROCESSED_CS_PARSED_ROOT
    global PREPROCESSED_CS_SUMMARY_PATH
    global TARGET_GENERATOR_MAX_DRAWINGS_PER_CLASS

    if "NOTEBOOK_NAME" not in overridden_keys:
        NOTEBOOK_NAME = f"VQ_SGen_ver{NOTEBOOK_VER}"
    if "DRIVE_MODEL_ROOT" not in overridden_keys:
        DRIVE_MODEL_ROOT = f"{DRIVE_PROJECT_ROOT}/모델"
    if "DRIVE_MODEL_VER_DIR" not in overridden_keys:
        DRIVE_MODEL_VER_DIR = f"{DRIVE_MODEL_ROOT}/ver{NOTEBOOK_VER}"
    if "WORKSPACE_ROOT" not in overridden_keys:
        WORKSPACE_ROOT = f"/content/{NOTEBOOK_NAME}_workspace"
    if "RUN_TAG" not in overridden_keys:
        RUN_TAG = f"{NOTEBOOK_NAME}_{RUN_MODE}_{RUN_SUFFIX}"
    if "PREPROCESSED_CS_PARSED_ROOT" not in overridden_keys:
        PREPROCESSED_CS_PARSED_ROOT = f"{DRIVE_PROJECT_ROOT}/코드2/creativesketch/parsed/creativesketch"
    if "PREPROCESSED_CS_SUMMARY_PATH" not in overridden_keys:
        PREPROCESSED_CS_SUMMARY_PATH = f"{DRIVE_PROJECT_ROOT}/코드2/preprocess_summary.json"
    if "TARGET_GENERATOR_MAX_DRAWINGS_PER_CLASS" not in overridden_keys:
        TARGET_GENERATOR_MAX_DRAWINGS_PER_CLASS = TARGET_REPRESENTATION_MAX_DRAWINGS_PER_CLASS


_OVERRIDDEN_KEYS = _load_external_config_overrides()
_apply_derived_defaults(_OVERRIDDEN_KEYS)
...
env_cache_dir = os.environ.get("QUICKDRAW_CACHE_DIR", "").strip()
if env_cache_dir:
    download_dir = str(Path(env_cache_dir).expanduser().resolve())
elif str(QUICKDRAW_CACHE_DIR).strip():
    download_dir = str(Path(str(QUICKDRAW_CACHE_DIR).strip()).expanduser().resolve())
else:
...
def main():
    ...
    save_trained_artifacts(cfg, results)
